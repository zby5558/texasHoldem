# -*- coding: utf-8 -*-
"""
Created on Mon May  4 19:13:06 2020
...
def listen(ip, port, incoming_buffer):
    ...
    while True:
        ...
        try:
            ...
            print('time out')
def TestServer(): #create a number of virtual tables in the server
    for i in range(1): #range(1) means there is only one table, if you want k tables, please modify it to "for i in range(k):"
        for j in range(10):#add 10 virtual players to rach table
            gamerID[i].append(str(i*10+j))
            gamerName[i].append(str(i*10+j))
            gamerIP[i].append('192.168.1.14')# the ip address of the client, need to be modified
            gamerChip[i].append(100000000)
    for rID in range(1):#start the game for the virtual tables, if there are k tables, please modify it ot "for rID in range(k)"
        t = threading.Thread(target=gameStart,args=(rID,gamerID, gamerChip , gameOwner, gamerIP, gamerName, fiveCards, cardInHand,cardLeft))
        t.start()        
...
t2.start()
t3 = threading.Thread(target=TestServer,)
t3.start()
